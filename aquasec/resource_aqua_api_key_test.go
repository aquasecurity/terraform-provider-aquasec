...
func TestAquasecAPIKey_basic(t *testing.T) {
	...
	descriptionCreate := "Created using terraform for test API Key"
	enabledCreate := false
	rolesCreate := []string{"role1"}
	ip_addressesCreate := []string{"192.168.1.2"}
	expirationCreate := 60

	descriptionUpdate := "Updated using terraform for test API Key"
	enabledUpdate := false
...
func testAccAPIKeyDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "aquasec_api_key.test" {
			continue
		}
