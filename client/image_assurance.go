...
// GetImageAssurancePolicy - returns single Image Assurance Policy
func (cli *Client) GetImageAssurancePolicy(name string) (*ImageAssurancePolicy, error) {
	var err error
	var response ImageAssurancePolicy
	cli.gorequest.Set("Authorization", "Bearer "+cli.token)
	apiPath := fmt.Sprintf("/api/v2/assurance_policy/image/%s", name)
	resp, body, errs := cli.gorequest.Clone().Get(cli.url + apiPath).End()
	if errs != nil {
		return nil, errors.Wrap(getMergedError(errs), "failed getting Image Assurance Policy")
	}
	if resp.StatusCode == 200 {
		err = json.Unmarshal([]byte(body), &response)
		if err != nil {
			log.Printf("Error calling func GetImageAssurancePolicy from %s%s, %v ", cli.url, apiPath, err)
			return nil, err
		}
	} else {
		...
		return nil, fmt.Errorf("failed getting Image Assurance Policy. status: %v. error message: %v", resp.Status, errorResponse.Message)
	}

	if response.Name == "" {
		return nil, fmt.Errorf("Image Assurance Policy not found: %s", name)
	}
	return &response, err
}
...
func (cli *Client) CreateImageAssurancePolicy(imageassurancepolicy *ImageAssurancePolicy) error {
	...
	if resp.StatusCode != 201 && resp.StatusCode != 204 {
		...
		return fmt.Errorf("failed creating Image Assurance Policy. status: %v. error message: %v", resp.Status, errorResponse.Message)
	}
	return nil
}
...
func (cli *Client) UpdateImageAssurancePolicy(imageassurancepolicy *ImageAssurancePolicy) error {
	...
	if resp.StatusCode != 201 && resp.StatusCode != 204 {
		...
		return fmt.Errorf("failed modifying Image Assurance Policy. status: %v. error message: %v", resp.Status, errorResponse.Message)
	}
	return nil
}

// DeleteImageAssurancePolicy removes a Image Assurance Policy
func (cli *Client) DeleteImageAssurancePolicy(name string) error {
	request := cli.gorequest
	request.Set("Authorization", "Bearer "+cli.token)
	apiPath := fmt.Sprintf("/api/v2/assurance_policy/image/%s", name)
	resp, _, errs := request.Clone().Delete(cli.url + apiPath).End()
	if errs != nil {
		return errors.Wrap(getMergedError(errs), "failed deleting Image Assurance Policy")
	}
	if resp.StatusCode != 204 {
		...
		return fmt.Errorf("failed deleting Image Assurance Policy, status: %v. error message: %v", resp.Status, errorResponse.Message)
	}
	return nil
}