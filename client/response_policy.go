...
type ResponsePolicy struct {
	...
	Trigger          *ResponsePolicyTrigger `json:"trigger"`            // single object
	ApplicationScope []string               `json:"application_scopes"` // required
	Outputs          []ResponsePolicyOutput `json:"outputs"`            // required
...
func (cli *Client) GetResponsePolicyConfig() (*ResponsePolicyConfig, error) {
	...
	if events.StatusCode == 200 || events.StatusCode == 201 {
		...
		// No content - return nil response to caller to allow callers to handle absent body
		return nil, nil
	} else {
...
func (cli *Client) GetResponsePolicies(policyResp ResponsePolicyResp) (*ResponsePolicyResp, error) {
	...
	// Handle No Content explicitly to avoid unmarshalling empty body
	...
	err = json.Unmarshal([]byte(body), &response)
	if err != nil {
		// Check if the response is an array (empty or with policies)
...
func (cli *Client) GetResponsePolicy(id string) (*ResponsePolicy, error) {
	...
	if events.StatusCode == 204 {
		// No content - treat as not found/empty
		return nil, nil
	}
...
func (cli *Client) DeleteResponsePolicies() (*map[string]interface{}, error) {
	...
	if events.StatusCode == 204 {
		// No content - return nil to indicate nothing to process
		return nil, nil
	}

	// If we reach here, the status code was not 200/201/204 — return an explicit error.
	err = fmt.Errorf("error calling %s, status code %d, body %s", apiPath, events.StatusCode, body)
	return nil, err
}
...
func (cli *Client) UpdateResponsePolicyStatus(id int, enabled bool) (*ResponsePolicy, error) {
	...
	if events.StatusCode == 204 {
		// No content - return nil to indicate no response body
		return nil, nil
	}

	// Unexpected status code — return error with body for debugging.
	err = fmt.Errorf("error calling %s, status code %d, body %s", apiPath, events.StatusCode, body)
	return nil, err
}
